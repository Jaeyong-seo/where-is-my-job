...
FOR EACH ROW WHEN
 ...
 OR EXISTS (SELECT 1 FROM batch_policies p LEFT JOIN candidate_assertions a ON a.id=p.assertion_snapshot_id LEFT JOIN assertion_events e ON e.id=p.candidate_confirmation_event_id LEFT JOIN kill_switches k ON k.id=p.global_kill_switch_id LEFT JOIN capabilities c ON c.id=p.fixture_capability_id WHERE a.id IS NULL OR e.id IS NULL OR k.id IS NULL OR c.id IS NULL OR a.profile_id IS NOT p.candidate_profile_id OR a.state IS NOT 'active' OR e.profile_id IS NOT p.candidate_profile_id OR e.assertion_id IS NOT p.assertion_snapshot_id OR e.event_kind IS NOT 'confirmed' OR k.scope_kind IS NOT 'global' OR k.scope_key IS NOT 'global' OR k.state IS NOT 'closed' OR p.environment IS NOT 'fixture' OR p.timezone IS NOT '{{POLICY_TIMEZONE}}' OR (p.daily_cap IS NULL OR p.daily_cap NOT BETWEEN 1 AND 20) OR c.environment IS NOT 'fixture' OR c.state IS NOT 'active' OR c.adapter_id IS NOT p.fixture_adapter_id OR c.origin IS NOT p.fixture_origin)
...
FOR EACH ROW WHEN NOT EXISTS (
 ...
 WHERE p.id=NEW.policy_id AND p.timezone='{{POLICY_TIMEZONE}}'
