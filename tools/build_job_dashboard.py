...
from user_config import STATUSES, TIERS, TRACKER_PATH, load_profile, resume_pdf_name

ROOT = Path(__file__).resolve().parents[1]
DATA_PATH = TRACKER_PATH
...
.sync-banner { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; margin-bottom: 12px; padding: 12px 14px; border: 1px solid rgba(240,140,0,.25); border-radius: 16px; background: var(--orange-soft); color: #8c5700; font-size: 13px; }
.sync-banner strong { font-weight: 760; }
.sync-chip { padding: 4px 9px; border-radius: 999px; background: rgba(255,255,255,.7); font-size: 12px; font-weight: 650; }
.status-select.changed { border-color: var(--orange); box-shadow: 0 0 0 2px rgba(240,140,0,.14); }
...
<header class="topbar">
  <div class="topbar-inner">
    ...
    <nav class="top-actions" aria-label="Dashboard tools">
      <a class="quiet-button desktop-only" id="navSource" href="jobs/tracker.json">Source data</a>
      <a class="quiet-button desktop-only" id="navMaster" href="applications/_master/resume.md">Master resume</a>
...
<main class="shell">
  ...
  <section class="section" aria-labelledby="pipeline-title">
    ...
    <div class="filter-panel" aria-label="Posting filters">
      ...
      <select class="filter-select" id="sortBy" aria-label="Sort order">
        <option value="score">Sort: Fit score</option><option value="posted">Sort: Newest</option>
      </select>
    </div>
  ...
  <details class="automation" id="automationPanel">
    <summary>
...
const isLoopbackOrigin = window.location.protocol === 'http:' && window.location.hostname === '127.0.0.1';
const staticMode = !isLoopbackOrigin;
const remoteMode = staticMode && window.location.protocol !== 'file:';
const statusOverrides = data.status_overrides || {};
...
const currentStatus = role => {
  if (isConnected) return connectedRoles[role.id]?.status;
  const base = statusOverrides[role.id]?.status || role.status;
  if (artifactMode || !staticMode) return base;
  return saved[role.id] || base;
};
...
const materialsAvailable = () => isConnected || !remoteMode;
...
function setStatus(id, status, announce = true) {
  ...
  if (artifactMode) { persistSharedStatus(id, status); return; }
...
const embedJson = value => JSON.stringify(value).replace(/</g, '\\u003c').replace(/&/g, '\\u0026').replace(/\u2028/g, '\\u2028').replace(/\u2029/g, '\\u2029');
async function persistSharedStatus(id, status) {
  const role = roles.find(item => item.id === id);
  if (!role || !artifactApi) return;
  const selects = document.querySelectorAll(`[data-status-id="${CSS.escape(id)}"]`);
  selects.forEach(select => { select.disabled = true; });
  showToast('Saving to the shared page…');
  try {
    const response = await fetch(window.location.href, {cache:'no-store'});
    if (!response.ok) throw new Error('source unavailable');
    const source = await response.text();
    const marker = '<script id="job-data" type="application/json">';
    const start = source.indexOf(marker);
    const end = source.indexOf('<\/script>', start);
    if (start < 0 || end < 0) throw new Error('data block missing');
    const payload = JSON.parse(source.slice(start + marker.length, end));
    const overrides = {...(payload.status_overrides || {})};
    const baseline = (payload.roles.find(item => item.id === id) || {}).status;
    if (baseline === status) delete overrides[id];
    else overrides[id] = {status, at: new Date().toISOString(), from: baseline};
    payload.status_overrides = overrides;
    const updated = source.slice(0, start + marker.length) + embedJson(payload) + source.slice(end);
    await artifactApi.publish(updated);
    showToast(`${role.company} saved to the shared page for every viewer.`);
  } catch (error) {
    const code = String(error?.code || error?.message || error);
    if (code.includes('not_writer') || code.includes('not_granted')) {
      artifactReadOnly = true;
      showToast('This shared page is read-only for you.');
    } else if (code.includes('conflict')) {
      showToast('Someone else saved first — loading their version.');
    } else {
      saved[id] = status;
      writeStoredStatuses();
      showToast('Could not save to the shared page — kept in this browser only.');
    }
    renderAll();
    renderModeSurface();
  } finally {
...
function statusSelect(role, className = 'status-select') {
  const editable = !isConnected && staticMode && !artifactReadOnly;
  const disabled = editable ? '' : ' disabled title="Read-only while in service mode."';
  const changed = statusOverrides[role.id] ? ' changed' : '';
  ...
  return `<select class="${className}${changed}" data-status-id="${esc(role.id)}" aria-label="${esc(role.company)} ${esc(role.title)} application status"${disabled}>${options}</select>`;
}
...
function renderModeSurface() {
  ...
  const copy = document.getElementById('statusStorageCopy');
  if (artifactMode) {
    modeLabel.textContent = artifactReadOnly ? 'Shared page · read-only for you' : 'Shared page · saves for every viewer';
    modeLabel.classList.add('connected');
    copy.textContent = artifactReadOnly ? 'You can view this shared page but not change statuses.' : 'Status changes publish a new version of this page for every viewer. Ask Claude to sync them into the tracker.';
    document.getElementById('resetBtn').hidden = true;
  } else if (remoteMode) {
    modeLabel.textContent = 'Remote copy · this browser only';
    copy.textContent = 'Status changes stay in this browser and are not the repo record.';
  }
}
function renderPendingSync() {
  const banner = document.getElementById('syncBanner');
  if (!banner) return;
  const entries = Object.entries(statusOverrides);
  banner.hidden = entries.length === 0;
  if (!entries.length) { banner.innerHTML = ''; return; }
  banner.innerHTML = `<strong>${entries.length} change${entries.length > 1 ? 's' : ''} awaiting tracker sync</strong>` + entries.map(([id, info]) => {
    const role = roles.find(item => item.id === id);
    return `<span class="sync-chip">${esc(role ? role.company : id)}: ${esc(labelFor(info.from || (role || {}).status))} → ${esc(labelFor(info.status))}</span>`;
  }).join('');
}
function updateStatusFilterCounts() {
  const counts = {};
  roles.forEach(role => { const status = currentStatus(role); counts[status] = (counts[status] || 0) + 1; });
  document.querySelectorAll('#statusFilter option').forEach(option => {
    option.textContent = option.value === 'all' ? `All statuses (${roles.length})` : `${labelFor(option.value)} (${counts[option.value] || 0})`;
  });
}
function applyRemoteChrome() {
  if (!remoteMode) return;
  ['navSource', 'navMaster'].forEach(id => { const node = document.getElementById(id); if (node) node.hidden = true; });
...
function initArtifactContext() {
  if (!remoteMode || !window.claude || typeof window.claude.use !== 'function') return;
  Promise.resolve(window.claude.use('artifact')).then(api => {
    if (!api) return;
    artifactApi = api;
    artifactMode = true;
    renderAll();
...
function renderSpotlight() {
  ...
  container.innerHTML = `<div class="spotlight-grid"><div><div class="spotlight-kicker"><span class="live-dot"></span>Apply to this first · fit ${esc(role.score)}</div><div class="spotlight-company">${esc(role.company)}</div><h2>${esc(role.title)}</h2><p class="spotlight-reason">${esc(role.match)}</p><div class="spotlight-meta"><span class="glass-chip">${esc(role.location)}</span><span class="glass-chip">${esc(role.work_model)}</span><span class="glass-chip">${esc(role.salary)}</span></div></div><div class="spotlight-actions">${queueControl(role, 'primary-button') || externalApply(role, 'primary-button')}<button class="secondary-button" type="button" data-open-role="${esc(role.id)}">View materials</button>${materialsAvailable() ? `<a class="secondary-button" href="${esc(materialUrl(role))}">Resume PDF</a>` : ''}</div></div>`;
}
...
function renderRoles() {
  ...
  const byScore = (a,b) => b.score - a.score || b.posted.localeCompare(a.posted);
  const byPosted = (a,b) => b.posted.localeCompare(a.posted) || b.score - a.score;
  const filtered = roles.filter(roleMatches).sort(sortBy === 'posted' ? byPosted : byScore);
  ...
  document.getElementById('roleList').innerHTML = filtered.map(role => {
    ...
    const materialLink = materialsAvailable() ? `<a class="row-action material-link" href="${esc(materialUrl(role))}" aria-label="Open ${esc(role.company)} materials" title="Open materials">${icons.document}</a>` : '';
    return `<article class="role-row"><div class="score-ring ${scoreClass}" aria-label="Fit ${esc(role.score)}">${esc(role.score)}</div><div class="role-copy"><div class="role-company">${esc(role.company)}</div><div class="role-title">${esc(role.title)}</div><div class="role-match">${esc(role.match)}</div></div><div class="meta-stack"><div class="meta-primary">${esc(role.location)}</div><div class="meta-secondary">${esc(role.work_model)} · ${esc(role.salary)}</div></div>${statusSelect(role)}<div class="row-actions">${materialLink}${queue || externalApply(role, 'row-action')}<button class="row-action" type="button" data-open-role="${esc(role.id)}" aria-label="${esc(role.company)} posting details" title="Posting details">${icons.detail}</button></div></article>`;
  }).join('');
}
